...
SELECT 
  ...
  restaurant.telephone AS restaurant_telephone
FROM restaurant
JOIN address
...
ON
  categories_dishes.category_id = category.id
WHERE
  dish.hot_and_spicy = 't';

-- Shows how much a dish appears in the categories.
SELECT
  dish_id,
...
-- Shows only the dishes who appears in more than one category and how much appears
SELECT
  dish_id,
  COUNT(dish_id) AS dish_count
FROM
...
-- Same query than before but this shows the dish name instead of the id
SELECT
  dish.name,
